...
public class EnemyController_Photon : MonoBehaviour
{
    [System.Serializable]
    ...
    public WeaponController_Photon weapon;
    ...
    EnemyManager_Photon m_EnemyManager;
    ActorsManager m_ActorsManager;
    Health m_Health;
    Actor m_Actor;
    float m_TimeLastSeenTarget = Mathf.NegativeInfinity;
    Collider[] m_SelfColliders;
    GameFlowManager_Photon m_GameFlowManager;
    bool m_WasDamagedThisFrame;
    public bool ghost { get; set; }

    void Start()
    {
        m_EnemyManager = FindObjectOfType<EnemyManager_Photon>();
        DebugUtility.HandleErrorIfNullFindObject<EnemyManager_Photon, EnemyController_Photon>(m_EnemyManager, this);

        m_ActorsManager = FindObjectOfType<ActorsManager>();
        DebugUtility.HandleErrorIfNullFindObject<ActorsManager, EnemyController_Photon>(m_ActorsManager, this);

        m_EnemyManager.RegisterEnemy(this);

        m_Health = GetComponent<Health>();
        DebugUtility.HandleErrorIfNullGetComponent<Health, EnemyController_Photon>(m_Health, this, gameObject);

        m_Actor = GetComponent<Actor>();
        DebugUtility.HandleErrorIfNullGetComponent<Actor, EnemyController_Photon>(m_Actor, this, gameObject);

        m_NavMeshAgent = GetComponent<NavMeshAgent>();
        m_SelfColliders = GetComponentsInChildren<Collider>();

        m_GameFlowManager = FindObjectOfType<GameFlowManager_Photon>();
        DebugUtility.HandleErrorIfNullFindObject<GameFlowManager_Photon, EnemyController_Photon>(m_GameFlowManager, this);
    ...
    void OnDamaged(float damage, GameObject damageSource)
    {
        ...
        if (damageSource && damageSource.GetComponent<PlayerCharacterController_Photon>())
        {
            // pursue the player
            m_TimeLastSeenTarget = Time.time;
